...
parser.add_argument('--model', default='StyleID')
parser.add_argument('--tar', default = './output')
parser.add_argument('--sty', default = './data/sty_eval')
opt = parser.parse_args()
sty_path = opt.tar
tar_path = opt.sty
sty_list = get_image_list(sty_path)
tar_list = get_image_list(tar_path)
...
with open('eval_results.txt', 'a') as f:
        f.write(f'Model: {opt.model}, color matching loss:{(sum(hist_list) / len(hist_list)).item()}\n')
