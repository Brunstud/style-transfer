...
class PatchSimi(nn.Module):
    ...
    def get_feats(self, img):
        ...
        for name, layer in self.model._modules.items():
            ...
            if name in self.layers:
                # features.append(img)
                resized = F.interpolate(img, size=(128, 128), mode='bilinear', align_corners=False)
    ...
    def forward(self, input, target):
        ...
        for idx in range(len(src_feats)):
            cnt_feat = self.patch_simi_cnt(src_feats[idx])
            tgt_feat = self.patch_simi_out(target_feats[idx])
            if cnt_feat.shape != tgt_feat.shape:
                print(f"[Warning] Skipped due to shape mismatch: {cnt_feat.shape} vs {tgt_feat.shape}")
                # tgt_feat = F.interpolate(target_feats[idx], size=cnt_feat.shape[-2:], mode='bilinear', align_corners=False)
                continue
            init_loss += F.kl_div(cnt_feat, tgt_feat, reduction='batchmean')
        return init_loss
